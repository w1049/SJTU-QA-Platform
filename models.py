...
class Question(db.Model):
    ...
    belongs = db.relationship('QuestionSet', secondary=set2question, back_populates='questions')
...
class QuestionSet(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.String(255))
    questions = db.relationship('Question', secondary=set2question, back_populates='belongs')
    owner_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    owner = db.relationship('User', backref='own', uselist=False, foreign_keys=[owner_id])
    maintainer = db.relationship('User', secondary=set2user, backref='maintain')
    modified_at = db.Column(db.DateTime, default=datetime.now)
    modified_by_id = db.Column(db.Integer, db.ForeignKey('user.id'))
